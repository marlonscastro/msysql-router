...
RUN set -x \
	&& apt-get update && apt-get install -y --no-install-recommends ca-certificates lsb-release && rm -rf /var/lib/apt/lists/* \
...
# DIRETIVA VOLUME PERDE EFEITO QUANDO SE FAZ O DEPLOY NO OPENSHIFT
